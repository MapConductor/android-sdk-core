...
/**
 * 既定マーカーアイコンの土台。ピン形の輪郭・ラベル・キャッシュを受け持ち、
 * 中身の塗り方だけをサブクラスに任せる。
 *
 * `toBitmapIcon()` の結果は [BitmapIconCache] に入れる。同じ見た目のアイコンは
 * 何千個のマーカーで共有されるので、毎回 Bitmap を作ると描画より確保が重くなる。
 * キャッシュキーは見た目を決める全プロパティの hash（`getUniqueProperties()` を含む）。
 */
